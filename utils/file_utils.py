...
def get_file_extension_from_url(url):
    ...
    # If it's a Moodle pluginfile URL, try to get the real filename
    if "pluginfile.php" in path:
        # Check query parameters for filename
        if 'file' in query:
            filename = query['file'][0]
        elif 'forcedownload' in query:
            filename = query['forcedownload'][0]
        else:
            # The real filename is usually after pluginfile.php
            parts = path.split('pluginfile.php/')
            if len(parts) > 1:
    ...
    if extension and extension.lower() != '.php':  # Ignore .php extensions
...
def get_file_extension_from_headers(headers):
    ...
    content_type = headers.get('content-type', '').lower().split(';')[0]  # Remove charset part
